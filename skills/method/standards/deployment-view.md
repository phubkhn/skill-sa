...
**Artifact:** `docs/architecture/hld/deployment-<environment>.puml` plus the node catalogue, unless the repository has an established architecture-documentation layout.
...
Produced by `/sa:hld` when the deployment view is justified by the tailoring guidance or requested explicitly.
...
4. **Every network hop has a latency budget** consistent with the measurable end-to-end driver in the architecture brief. Hops whose sum exceeds the target are a finding, not a rounding error.
...
Follow `diagrams.md`, with these additions:
...
Self-assess against this list before reporting the artifact done. Report failed or not-applicable items; a failed item becomes an OPEN item with an owner.
...
- [ ] Consistent with the architecture brief's resilience assumptions and zone/region loss behaviour
